...
// Note: gemini-2.5-flash-image is the image generation variant, more stable than gemini-2.0-flash-exp
...
function shouldUseApiKey(modelName) {
  ...
  // All other models (including gemini-2.5-flash-image and gemini-2.0-flash-exp) use Vertex AI
...
async function generateSingleImage(prompt, referenceImageBuffers, category = null, retries = RATE_LIMIT.maxRetries) {
  return enqueueRequest(async () => {
    ...
    try {
      ...
      const responseModalities = (selectedModel === 'gemini-2.5-flash-image' || selectedModel === 'gemini-2.0-flash-exp')
        ? ['image', 'text']
        : ['image'];
